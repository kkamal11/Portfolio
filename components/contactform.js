const form = {
                ...
                <form name='submit-to-google-sheet'>
                    <div class="sm:text-left mx-8 md:mx-28">
                        <div class="mb-6">
                            ...
                            <p v-if="nameWarning.length > 0" class="bg-red-400 mt-2 p-2 rounded text-white">{{nameWarning}}<button v-on:click="cancelPopUp" class="float-right text-xl mr-2 font-bold hover:text-red-800 "><i class="bi bi-x-lg"></i></button></p>
                        </div>
                        <div class="mb-6">
                            <label for="email" class="block mb-2 text-base font-medium text-gray-700 dark:text-gray-300">Your email<span class="text-red-600">*</span> <i v-if="showCorrectEmailTick" class="bi bi-check2-circle text-xl text-white"></i> </label>
                            ...
                            <p v-if="emailwarning.length > 0" class="bg-red-400 mt-2 p-2 rounded text-white">{{emailwarning}}<button v-on:click="cancelPopUp" class="float-right text-xl mr-2 font-bold hover:text-red-800 "><i class="bi bi-x-lg"></i></button></p>
                        </div>
                        <div class="mb-4">
                            <label for="message" class="block mb-2 text-base font-medium text-gray-700 dark:text-gray-300">Your message<span class="text-red-600">*</span> <i v-if="showCorrectMessageTick" class="bi bi-check2-circle text-xl text-white"></i></label>
                            ...
                            <p v-if="email.length == 0 || message.length == 0" class="text-xs text-red-600 pt-1">Fields marked * are mandatory.</p>
                            <p v-if="textwarning.length > 0" class="bg-red-400 mt-2 p-2 rounded text-white" >{{textwarning}}<button v-on:click="cancelPopUp" class="float-right text-xl mr-2 font-bold hover:text-red-800"><i class="bi bi-x-lg"></i></button></p>
                        </div>
                        <p v-if="showThankYouMsg && !messageBeingSent && !showErrorMessage" class="bg-red-600 mt-2 p-2 rounded text-white"> Thank You For Getting In Touch.<button v-on:click="cancelPopUp" class="float-right text-xl mr-2 font-bold hover:text-red-800"><i class="bi bi-x-lg"></i></button></p>
                        <button v-if="showNewMessage && !messageBeingSent && !showErrorMessage" disabled class="green p-2 my-3 shadow-lg shadow-sky-800/50 text-white rounded"  :class="{'cursor-not-allowed': submitButtonDisabled }">Message sent successfully. ✔</button>
                        <button v-if="showErrorMessage" disabled class="bg-red-700 p-2 my-3 shadow-lg shadow-sky-800/50 text-white rounded"  :class="{'cursor-not-allowed': submitButtonDisabled }">Oops... An error occured.</button>
                        <button v-if="showNewMessage || showErrorMessage" v-on:click="reload" id="btn2" class="p-2 my-3 bg-yellow-400 shadow-lg shadow-sky-800/50 text-white rounded hover:bg-yellow-600">Send Another <i class="bi bi-chat-left-heart"></i></button>
    ...
    data() {
        return {
            userName: "",
            email: "",
            message: this.$route.query.title ? `Hey Kamal, I would love to discuss and know more about your project titled "${this.$route.query.title}".` : "",
